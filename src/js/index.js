  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                padding: 10px;
            `;
      form.insertAdjacentElement('afterend', statusMessage); // create form body for send meassage

      const formData = new FormData(form); // convert to object the formdata old method
      // const object = {};
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  const modal = document.querySelector(modalSelector),
        ...
        modalBtnClose = document.querySelectorAll('[data-close]'); // let modalStyle = window.getComputedStyle(modal).diplay;
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
      openModal(modalSelector, modalTimerId); // remove the listener if scroll to end
...
function slider({
  ...
  let sliderIndex = 1; // Assign current value in numeric of current slide
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 4000); // slider = require('./modules/slider')
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__.default)('.timer', '2021-11-12');
